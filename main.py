...
class Personnage:
    def __init__(self, user_id, nom, classe_nom, charger_db=False):
        ...
        if not charger_db:
            # --- C'EST ICI QU'IL MANQUAIT L'INITIALISATION ---
            self.niveau = 1          # Indispensable pour éviter ton erreur
            self.pv_actuel = 0
            self.mana = 0
            self.tension = 0
            self.ferveur = 0
            ...
            # Attributs RP (valeurs par défaut)
            self.oral = 0
            self.force_rp = 0
            self.survie = 0
            self.histoire = 0
            ...
            self.init_stats_depart()   # Définit Force, Agi, etc. selon la classe
            self.recalculer_derives()  # Utilise self.niveau (maintenant ça marche !)
            
            # On remplit les PV au max à la création
