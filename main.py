...
async def defense(interaction: discord.Interaction, type_def: app_commands.Choice[str], degats_subis: int, ressource_spend: int = 0, inversion: bool = False):
    ...
    if degats_finaux > 0:
        ...
        # Si on prend des dégâts, la stabilité baisse
...
@bot.tree.command(name="fiche", description="Voir votre fiche, stats et compétences triées")
async def fiche(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title=f"📜 {p.nom} (Niv {p.niveau} {p.classe.capitalize()})", color=0x3498db)
    
    # --- Bloc 1 : Stats (Inchangé) ---
    ...
    # --- NOUVEAU BLOC : COMPÉTENCES TRIÉES ---
    liste_tronc = []
    liste_spe = []
    liste_passifs = []

    # On parcourt les compétences apprises par le joueur
    for skill_key in p.competences:
        if skill_key in SKILLS_DB:
            data = SKILLS_DB[skill_key]
            nom_sort = f"🔹 {data['nom']} (P{data['pallier']})"
            
            # Tri
            if data.get('type') == 'passif':
                liste_passifs.append(f"🔸 {data['nom']}") # Icône différente pour les passifs
            elif data.get('cat') == 'spe':
                liste_spe.append(nom_sort)
            else:
                # Par défaut, on met dans le tronc commun
                liste_tronc.append(nom_sort)

    # Affichage conditionnel (si la liste n'est pas vide)
    if liste_tronc:
        embed.add_field(name="📘 Tronc Commun", value="\n".join(liste_tronc), inline=True)
    
    if liste_spe:
        embed.add_field(name="📕 Sous-Classe / Spé", value="\n".join(liste_spe), inline=True)
        
    if liste_passifs:
        embed.add_field(name="🛡️ Passifs", value="\n".join(liste_passifs), inline=False)
